...
public:
	...
	void stamp(letter l, int x, int y, int z, unsigned char color[4], std::vector<unsigned char> &data, int dim) {
		...
		for(int yy = 0; yy < ydim; yy++)
			if(l.data[xx][yy]==1)
				write(x+xx,y+yy,z,color,data,dim);
	...
	void populate(std::vector<unsigned char> &data, int dim, int count, float color[4]){
		...
		std::uniform_int_distribution<int> colo(-5, 5);
		...
		for(int i = 0; i < count; i++) {
			...
			stamp(letters[pick(gen)], loc(gen), loc(gen), loc(gen), col, data, dim);
		}
	}
};
